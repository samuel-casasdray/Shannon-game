    ...
    public Pane run() {
        ...
        for (int i = 0; i < nbVertex; i++) {
            ...
            if (aroundCircle) { // Coord autour d'un cercle
                double toRad = (i * 360f / nbVertex) * (Math.PI / 180);
        ...
        for (int i = 0; i < nbVertex; i++) {
            for (int j = i + 1; j < nbVertex; j++) {
                ...
                if (p > proba) {
                    ...
                    Line line = new Line(coords.get(i).getKey() + 20, coords.get(i).getValue() + 20, coords.get(j).getKey() + 20, coords.get(j).getValue() + 20);
